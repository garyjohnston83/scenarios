...
# IMPORTANT: Use the current working directory (project root) as base path.
# Do NOT create specs under agent-os-profiles or any --add-dir path.
PROJECT_ROOT="$(pwd)"
SPEC_PATH="$PROJECT_ROOT/agent-os/specs/$DATED_SPEC_NAME"
...
3. **Ask Clarifying Questions**: Generate targeted questions WITH visual asset request AND reusability check
   **CRITICAL:** You MUST ask clarifying questions using the `/ask-questions` skill BEFORE writing any requirements. Steps 1-3 below MUST be completed and you MUST invoke `/ask-questions` and STOP. Do NOT proceed to Steps 4-7 until the user has responded with answers. If this is a new session, your ONLY job is to ask questions — never skip ahead to writing `requirements.md`.
4. **Process Answers**: Analyze responses and any provided visuals
5. **Ask Follow-ups**: Based on answers and visual analysis if needed using the `/ask-questions` skill 
6. **Save Requirements**: Document the requirements you've gathered to a single file named: `[spec-path]/planning/requirements.md`

## MANDATORY: Ask Questions Before Anything Else
**CRITICAL:** You MUST ask clarifying questions [Step 3] using the `/ask-questions` skill BEFORE writing any requirements. Steps 1-3 below MUST be completed and you MUST invoke `/ask-questions` and STOP. Do NOT proceed to Steps 4-7 until the user has responded with answers. If this is a new session, your ONLY job is to ask questions — never skip ahead to writing `requirements.md`.
...
**OUTPUT these questions using the `/ask-questions` skill and STOP - wait for user response.**

Use the `/ask-questions` skill to format your questions with unique IDs:

```
/ask-questions
- [uuid-1] Based on your idea for [spec name], I assume [specific assumption]. Is that correct, or [alternative]?
- [uuid-2] I'm thinking [specific approach]. Should we [alternative]?
- [uuid-3] [Continue with questions...]
- [uuid-reuse] Are there existing features in your codebase with similar patterns we should reference?
- [uuid-visual] Do you have any design mockups, wireframes, or screenshots that could help guide the development?
```

Generate a unique ID for each question (can be simple like `q1`, `q2`, etc. or UUIDs).
...
**If follow-ups needed, use `/ask-questions` skill:**

```
/ask-questions
- [uuid-1] Based on your answers [and the visual files I found], [specific follow-up question]
- [uuid-2] [Another follow-up if needed]
```
...
- Use `/ask-questions` skill to format questions with unique IDs
